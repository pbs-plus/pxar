...
func TestIntegration_ChunkedDidxUploadDownload(t *testing.T) {
	...
	// Verify the index digest from the upload result is valid hex
	if len(hex.EncodeToString(result.Digest[:])) != 64 {
		t.Errorf("index digest has wrong length: %d", len(hex.EncodeToString(result.Digest[:])))
	}

	// Verify the returned digest matches what PBS expects
	// The index digest is computed by PBS and returned - verify it matches our computed value
	csum, _ := idx.ComputeCsum()
	if csum != result.Digest {
		t.Logf("Note: computed index checksum differs from PBS digest (PBS may use different computation)")
		t.Logf("  computed: %s", hex.EncodeToString(csum[:]))
		t.Logf("  PBS:      %s", hex.EncodeToString(result.Digest[:]))
	}
